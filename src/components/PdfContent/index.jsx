...
const PdfGenerator = ({ updatedButtonValues, coordenadores, diretores, turma, cabecalho, tipoEnsino, conceitoFinal, disciplinas, date, segundoSemestre}) => {
  ...
  const handleGeneratePDF = () => {
    ...
    // add aqui a condicional EJA segundo semestre
    else if(segundoSemestre){
      doc.text(`${semestre} BIMESTRE DE ${anoAtual} - ${tipoEnsino_parse}`, doc.internal.pageSize.getWidth() / 2, 140, { align: 'center' });
      doc.text(` ${turma}`, doc.internal.pageSize.getWidth() / 2, 160, { align: 'center' });
    }
     else {
    ...
    const splitRowsByPage = (rows, disciplinasLength) => {
      const firstPageRowsCount = disciplinasLength <= 13 ? 10 : 6;
      const subsequentPageRowsCount = disciplinasLength <= 13 ? 15 : 10;
    ...
    if(segundoSemestre){
      doc.text(`${semestre}º BIMESTRE DE ${anoAtual} - ${turma} - ${tipoEnsino_parse}`, doc.internal.pageSize.getWidth() / 2, 570, { align: 'center' });
    }else{
